...
-Activates the current item/tray.
-Waits for the weight to become stable.
-Confirms and saves the weight to that specific button/record.
-Moves to the next item in the chain automatically.

Smart Google Sheets Sync:
Automatically checks for internet connectivity after the final item is weighed.
Uses the Google Sheets API to locate the correct file.
Maps data to specific cells based on the current Month and Day.
